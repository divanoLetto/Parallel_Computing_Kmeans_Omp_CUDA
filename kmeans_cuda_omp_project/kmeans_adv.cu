...
__device__ float records_adv_distance(float x_1, float y_1, float x_2, float y_2) {
  return sqrt((x_1 - x_2) * (x_1 - x_2) + (y_1 - y_2) * (y_1 - y_2));
}

Record_adv * get_records_from_file(int NUM_RECORDS){
...
Record_adv * get_centroids_from_file( int k){
...
__global__ void  cluster_assigment(float* records_x,float *records_y,float*centroids_x,float*centroids_y,float*sum_x,float*sum_y,int num_rec,int k,int * sizes_centroid_d, int* assignment_d){
    ...
    if (threadIdx.x < k) {
        smem[threadIdx.x] = centroids_x[threadIdx.x];//shared_centroids_x
        smem[k+threadIdx.x] = centroids_y[threadIdx.x];//shared_centroids_y
    }
    __syncthreads();
    ...
    for (int cluster = 0; cluster < k; ++cluster) {
        const float distance = records_adv_distance(x,y,smem[cluster],smem[k+cluster]);
...
__global__ void relocations_centroids(float * centroids_x, float* centroids_y, float* sum_x, float* sum_y, int* sizes_centroid_d){
    const int cluster = threadIdx.x;
    int size = max(sizes_centroid_d[cluster], 1);

    //printf("Thread id %d has sum x: %f y: %f to divide by size %d\n", cluster, sum_x[cluster], sum_y[cluster], size);
...
__global__ void print_centroids(float * centroids_x, float* centroids_y){
    const int cluster = threadIdx.x;
    if(cluster==0){
...
__global__ void print_records(float * records_x, float* records_y){
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    if(index==0){
...
bool close_centroids_cuda(int j,float* old_centroids_x, float*  old_centroids_y, float*  old_centroids_tmp_x,float*  old_centroids_tmp_y,int NUM_CLUSTERS,float MIN_TOLLERANCE){
    if(j==0){
        return false;
    }
...
int kmeans_cuda(float ** host_centroids_x, float ** host_centroids_y, float**host_records_x,float ** host_records_y, int**assignment, int* n_cluster, int *n_record,int NUM_RECORDS, int NUM_CLUSTERS, int MAX_ITERATIONS, float MIN_TOLLERANCE, int TPB)
{
    Record_adv * records = get_records_from_file(NUM_RECORDS);
    Record_adv * centroids = get_centroids_from_file(NUM_CLUSTERS);
    ...
    //print_centroids<<<1, NUM_CLUSTERS>>>(centroids->x,centroids->y);
    //print_records<<<1, NUM_RECORDS>>>(records->x,records->y);
    ...
    for(int i=0; i<MAX_ITERATIONS; i++){
        ...
        cluster_assigment<<<(NUM_RECORDS+TPB-1)/TPB,TPB, 2*NUM_CLUSTERS*sizeof(float)>>>(records->x,
                records->y,
                centroids->x,
                centroids->y,
                sum->x,
        ...
        if(close_centroids_cuda(i,old_centroids_x, old_centroids_y, old_centroids_tmp_x, old_centroids_tmp_y,NUM_CLUSTERS, MIN_TOLLERANCE)) {
            break;
        }
        ...
        relocations_centroids<<<1, NUM_CLUSTERS>>>(centroids->x,
                centroids->y,
                sum->x,
                sum->y,
                sizes_centroid_d );
